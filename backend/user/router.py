...
from user.service import signup_service, login_service, token_required, getuser_service, google_service
...
def callback():
  ...
  response.set_cookie('jwt_token', value=jwt_token, domain='127.0.0.1:5000')

  user = User.query.filter_by(public_id=id_info.get('sub')).first()
